...
{
	HAL_I2C_Mem_Write(&hi2c1, SH1106_Write, SH1106_Cmd, I2C_MEMADD_SIZE_8BIT,&Command, sizeof(Command), 10);
	
}
// 发送数据
volatile uint8_t OLED_DMA_Busy = 0; // DMA状态标志

void OLED_WriteData(uint8_t *Data, uint16_t Count) { //算了不搞DMA了 烦死了
//    taskENTER_CRITICAL();
//    volatile uint8_t isBusy = OLED_DMA_Busy;
//    taskEXIT_CRITICAL();
//    
//    while(isBusy) {
...
//    OLED_DMA_Busy = 1;
//    taskEXIT_CRITICAL();
 //HAL_I2C_Mem_Write_DMA(&hi2c1, SH1106_Write, SH1106_Data, I2C_MEMADD_SIZE_8BIT, Data, Count);
	
	
	HAL_I2C_Mem_Write(&hi2c1, SH1106_Write, SH1106_Data, I2C_MEMADD_SIZE_8BIT, Data, Count, 10);
}
...
void Init_OLED_Mutex(void) {
    if(xOLEDMutex == NULL) {
        xOLEDMutex = xSemaphoreCreateRecursiveMutex();
    }
}
void OLED_Init(void) {
	...
	OLED_Clear();				//清空显存数组
	OLED_UpdateP();				//更新显示，清屏，防止初始化后未显示内容时花屏
}
...
void OLED_Update(void) {
    if (xSemaphoreTakeRecursive(xOLEDMutex, portMAX_DELAY) == pdTRUE) {
        for (uint8_t j = 0; j < 8; j++) {
            OLED_SetCursor(j, 0);
            OLED_WriteData(OLED_DisplayBuf[j], 128); 
        }
        xSemaphoreGiveRecursive(xOLEDMutex);
    }
}

void OLED_UpdateP(void) {
        for (uint8_t j = 0; j < 8; j++) {
            OLED_SetCursor(j, 0);
            OLED_WriteData(OLED_DisplayBuf[j], 128); 
    }
...
void OLED_ShowChar(int16_t X, int16_t Y, char Char, uint8_t FontSize)
{
  if (xSemaphoreTakeRecursive(xOLEDMutex, portMAX_DELAY) == pdTRUE) {
		...
		// 原有显存操作代码...
					xSemaphoreGiveRecursive(xOLEDMutex); // 释放锁
		}
}
