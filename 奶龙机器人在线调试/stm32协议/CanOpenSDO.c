...
uint8_t CanOpen_UpdateInfo(void) {
    ...
    stat.speedL = -((int16_t)(speedRaw & 0xFFFF)) * 0.1; // 左电机
    stat.speedR = ((int16_t)((speedRaw >> 16) & 0xFFFF)) * 0.1; // 右电机
    ...
    targetSpeedRaw = (response[4] | (response[5] << 8) | (response[6] << 16) | (response[7] << 24));
		float scale = kResol2RPM[gSpeedResolution];   // Count/RPM
    stat.req_speedL = (float)(((int16_t)(targetSpeedRaw & 0xFFFF)) * scale);    // 左侧取反（电机反转）
    stat.req_speedR = (float)(((int16_t)((targetSpeedRaw >> 16)) & 0xFFFF) * scale);    // 右侧
